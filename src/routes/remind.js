...
router.get('/list', async (req, res) => {
    try {
        ...
        const reminders = getAll(sql, params);
        const total = getOne('SELECT COUNT(*) as count FROM reminders WHERE user_id = ? AND is_deleted = 0', [req.userId]);
        res.json({ code: 200, data: { list: reminders, total: total ? total.count : 0 } });
    } catch (error) {
...
router.get('/detail/:id', async (req, res) => {
    try {
        await initDb();
        const reminder = getOne('SELECT r.*, m.title as memo_title FROM reminders r LEFT JOIN memos m ON r.memo_id = m.id WHERE r.id = ? AND r.user_id = ?', [req.params.id, req.userId]);
        if (!reminder) {
...
router.post('/create', async (req, res) => {
    try {
        ...
        runQuery(`
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        `, [req.userId, memo_id || null, title, content || '', remind_time, repeat_type || 'once']);
        const newId = getLastInsertId();
        const newReminder = getOne('SELECT * FROM reminders WHERE id = ?', [newId]);
...
router.post('/update/:id', async (req, res) => {
    try {
        ...
        const reminder = getOne('SELECT * FROM reminders WHERE id = ? AND user_id = ?', [req.params.id, req.userId]);
        if (!reminder) {
            return res.json({ code: 404, message: '提醒不存在' });
        }
        runQuery(`
        ...
        const updatedReminder = getOne('SELECT * FROM reminders WHERE id = ?', [req.params.id]);
...
router.post('/complete/:id', async (req, res) => {
    try {
        ...
        runQuery('UPDATE reminders SET is_completed = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [newStatus, req.params.id]);
...
router.post('/delete/:id', async (req, res) => {
    try {
        await initDb();
        runQuery('UPDATE reminders SET is_deleted = 1 WHERE id = ? AND user_id = ?', [req.params.id, req.userId]);
...
router.post('/batchDelete', async (req, res) => {
    try {
        ...
        ids.forEach(id => {
            runQuery('UPDATE reminders SET is_deleted = 1 WHERE id = ? AND user_id = ?', [id, req.userId]);
        });
